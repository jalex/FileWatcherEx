    ...
    class EventProcessor
    {
        ...
        private IEnumerable<FileChangedEvent> NormalizeEvents(FileChangedEvent[] events) {
            ...
            foreach(var newEvent in events) {
                ...
                if(oldEvent != null && oldEvent.ChangeType == ChangeType.CREATED && newEvent.ChangeType == ChangeType.DELETED) { // CREATED + DELETED => remove
                    mapPathToEvents.Remove(oldEvent.FullPath);
                    eventsWithoutDuplicates.Remove(oldEvent);
                } else
                if(oldEvent != null && oldEvent.ChangeType == ChangeType.DELETED && newEvent.ChangeType == ChangeType.CREATED) { // DELETED + CREATED => CHANGED
                    oldEvent.ChangeType = ChangeType.CHANGED;
                } else
                if(oldEvent != null && oldEvent.ChangeType == ChangeType.CREATED && newEvent.ChangeType == ChangeType.CHANGED) { // CREATED + CHANGED => CREATED
                    // Do nothing
                } else { // Otherwise
                    
                    if(newEvent.ChangeType == ChangeType.RENAMED) { // If <ANY> + RENAMED
                        do {
                            mapPathToEvents.TryGetValue(newEvent.OldFullPath, out var renameFromEvent); // Try get event from newEvent.OldFullPath

                            if(renameFromEvent != null && renameFromEvent.ChangeType == ChangeType.CREATED) { // If rename from CREATED file
                                ...
                                if(oldEvent != null && oldEvent.ChangeType == ChangeType.DELETED) { // DELETED + CREATED => CHANGED
                                    newEvent.ChangeType = ChangeType.CHANGED;
                                }
                            } else
                            if(renameFromEvent != null && renameFromEvent.ChangeType == ChangeType.RENAMED) { // If rename from RENAMED file
                    ...
                    if(oldEvent != null) { // If old event exists
                    ...
                    } else { // If old event is not exist
                        // Add new event
        ...
        private IEnumerable<FileChangedEvent> NormalizeEvents(FileChangedEvent[] events)
        {
            ...
            {
                if(e.ChangeType == ChangeType.RENAMED && mapPathToEvents.TryGetValue()
                // Existing event
